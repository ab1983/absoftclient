...
import static com.algoboss.core.face.GenericBean.sendEmail;
...
import com.algoboss.core.dao.AdmInstantiatesSiteDao;
...
public class GerLoginBean implements Serializable {
    ...
    public void doInstatiatesSite() {
        instantiatesSiteList = baseDao.findByContractId(user.getContract().getContractId());
    ...
    private boolean doCheckUpdateAccount(){ 
    	URL url;
    	String devid = "";
    	action = "";
    	boolean toUpdate = (lastUpdate == null || (new Date().getTime() - lastUpdate.getTime()) > 5*60000);
    	boolean ivpk = false;
		try {
			//url = new URL("http://app.algoboss.com/update");		
				SecUser userActual = new UsuarioDao(baseDao).findByEmailAndPassword(user.getEmail().toUpperCase(), user.getPassword());
				if(userActual!=null && userActual.getContract()!=null){
					user = userActual;
					if(ivpk = AlgoUtil.isValidPrivateKey(userActual.getContract().getPrivateKey())){
						devid = userActual.getContract().getPrivateKey();
					}else{
						System.out.println("### ERROR TO COMPARE PRIVATE KEY CLIENT (INVALID PK)!");
						toUpdate = true;
					}
				}	
				...
				if(toUpdate){
					if(devid ==null || devid.isEmpty()){
						devid = AlgoUtil.getPrivateKey();
					}
					if(!devid.isEmpty()){
						String usrid ="usr:"+user.getLogin()+";"+"pwd:"+user.getPassword();
						HttpServletRequest request = (HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest();
						AdmContract contractToUp = new AdmContract();
						contractToUp.setPrivateKey(devid);
						contractToUp.setSystemContextPath(request.getRequestURL().toString());
						SecUser userToUp = new SecUser();
						userToUp.setEmail(user.getEmail());
						userToUp.setPassword(user.getPassword()); 
						userToUp.setContract(contractToUp);
						Gson gson = new Gson();	
						url = new URL("http://dev.algoboss.com/update");
						//url = new URL("http://192.168.1.200:8080/ABMASTER/update");
						//url = new URL("http://192.168.1.220:8080/ABMASTER/update");
						String urlParameters  = gson.toJson(userToUp);
						...
						try {
							try( DataOutputStream wr = new DataOutputStream( conn.getOutputStream())) {
								wr.write( new String(urlParameters).getBytes("ISO-8859-1"));
								wr.flush();
								wr.close();			   
							}  
							conn.connect();
							conn.getResponseCode();
							action = new String(AlgoUtil.getBytes(conn.getInputStream()),"ISO-8859-1");
						} catch (java.net.UnknownHostException e) {
							Logger.getLogger(GerLoginBean.class.getName()).log(Level.SEVERE,"########### HOST NOT FOUND! "+e.getMessage());
						}
					}else{
						//FALHA AO GERAR PRIVATE KEY
						return true;
					}
				}
    ...
    public String doUpdateAccount(){
    	...
    	try {
			...
			jsEngine.put("login", this);
			jsEngine.eval(action); 
			action = "";
			lastUpdate = new Date();
			return doLogin();
